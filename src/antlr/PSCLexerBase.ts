      ...
      case PSCLexer.NEWLINE:
        ...
        if (indentLevel > lastIndentLevel) {
          ...
          if (indentLevel - lastIndentLevel != 1) {
            throw new Error(`indentation level changed by more than one: ${lastIndentLevel} -> ${indentLevel} at line ${token.line}`)
          }
    ...
    switch (whitespace[i]) {
      case ' ':
        if (whitespace[i + 1] === ' ') {
          ...
          throw new Error(`expected two consecutive spaces for indentation, but found one at index ${i}`)
        }
        break
      case '\t':
        level++
